...
export default function CharactersPage() {
  ...
  const [unlockedTraits, setUnlockedTraits] = useState<NarrativeContent[]>([]);
  ...
  async function loadData() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setUnlockedTraits(contentData.characterTraits || []);
      }
    } catch {
    ...
    <div className="space-y-10">
      <div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <button
            ...
            className="btn-primary text-xs !py-2.5 !px-4 flex items-center justify-center gap-2 shrink-0 shadow-md shadow-indigo-500/20"
          >
      ...
      {loading ? (
        ...
        <div className="glass-card p-12 text-center text-slate-400 border-dashed space-y-4">
          <p>Nessun personaggio creato. Clicca su &quot;+ Nuovo Personaggio&quot; per iniziare.</p>
          <button type="button" onClick={handleStartCreate} className="btn-secondary text-xs">
            <Plus className="w-4 h-4 mr-1 inline" /> Crea Primo Personaggio
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {characters.map((char) => {
            ...
            return (
              <div key={char.id} className="glass-card p-5 flex flex-col justify-between gap-4">
                <div>
      ...
      {/* Form Dialog/Sheet per Aggiungere o Modificare Personaggio */}
      <ResponsiveModal
        open={isFormOpen}
        onOpenChange={(open) => {
      ...
      >
        <div className="pb-4">
          {children.length === 0 ? (
            ...
            {unlockedTraits.filter((t) => !t.isUnlocked).length > 0 && (
              <div className="p-3 rounded-xl bg-slate-900/40 border border-slate-700 opacity-70">
                <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">
                  🔒 Sblocca nuovi tratti nel{" "}
                  <Link href="/billing" className="text-amber-400 hover:text-amber-300 underline underline-offset-2">
                    Negozio Narrativo
                  </Link>
                </p>
