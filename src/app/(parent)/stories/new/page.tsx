...
export default function NewStoryPage() {
  ...
  const [storyStyles, setStoryStyles] = useState<NarrativeContent[]>([]);
  ...
  async function loadLibraryAndChildren() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStoryStyles(
          (contentData.storyStyles || []).filter(
            (s: NarrativeContent) => s.isUnlocked
          )
        );
      }
    } catch {
    ...
    <div className="max-w-4xl mx-auto space-y-8 relative">
      ...
      {generatedStory ? (
        ...
        <form onSubmit={handleGenerate} className="space-y-8">
          ...
          {/* 6. Stile Narrativo (Opzionale) */}
          <div className="glass-card p-6 space-y-4">
            <h2 className="font-bold text-lg text-white flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-amber-400" />
              <span>6. Stile Narrativo (Opzionale)</span>
            </h2>

            {storyStyles.length === 0 ? (
              <div className="p-3 rounded-xl bg-slate-900/40 border border-slate-700">
                ...
                >
                  <span>✨</span>
                  <span>
                    Sblocca stili narrativi esclusivi nel{" "}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {/* Nessuno stile (default) */}
                <button
                  type="button"
                  onClick={() => setSelectedStyleId("")}
                  className={`p-3.5 rounded-2xl border text-left transition-all ${
                    selectedStyleId === ""
                      ? "bg-slate-700/40 border-slate-500 text-white shadow-md"
                      : "bg-slate-900/60 border-slate-800 text-slate-400 hover:border-slate-700"
                  }`}
                >
                  <strong className="text-xs font-bold block mb-1">Stile Standard</strong>
                  <span className="text-[11px] text-slate-400 block">Nessuno stile speciale applicato</span>
                </button>
                ...
                {storyStyles.map((style) => {
                  ...
                  return (
                    <button
                      key={style.id}
                      type="button"
                      onClick={() => setSelectedStyleId(active ? "" : style.id)}
                      className={`p-3.5 rounded-2xl border text-left transition-all ${
                        active
                          ? "bg-amber-500/20 border-amber-500 text-white shadow-md shadow-amber-500/10"
                          : "bg-amber-500/10 border-amber-500/40 text-slate-300 hover:border-amber-500/60"
                      }`}
                    >
                      <strong className="text-xs font-bold block mb-1 text-amber-300">
                        ✨ {style.name}
                      </strong>
                      <span className="text-[11px] text-slate-400 block line-clamp-2">
                        {style.description}
